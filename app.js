...
window.openServiceForm=(cid="")=>{const opts=customers.map(c=>`<option value="${c.id}" ${c.id===cid?"selected":""}>${esc(c.ad_soyad)} - ${esc(c.telefon)}</option>`).join("");openModal(`<h2>Yeni Servis</h2><form id="sForm"><select id="sCustomer" required><option value="">Müşteri seç</option>${opts}</select><input id="sDate" type="date" value="${new Date().toISOString().slice(0,10)}" required><textarea id="sComplaint" placeholder="Müşteri şikâyeti / arıza"></textarea><textarea id="sDiagnosis" placeholder="Tespit / teşhis"></textarea><textarea id="sWork" placeholder="Yapılan işlem"></textarea><input id="sPart" placeholder="Değişen parça"><input id="sParts" type="number" min="0" step=".01" placeholder="Parça ücreti"><input id="sLabor" type="number" min="0" step=".01" placeholder="İşçilik ücreti"><select id="sPay"><option>Ödenmedi</option><option>Ödendi</option></select><select id="sStatus"><option>Tamamlandı</option><option>Bekliyor</option><option>Devam ediyor</option></select><input id="sPhoto" type="file" accept="image/*" multiple><label>📸 Servis fotoğrafları</label><h3>Müşteri İmzası</h3><canvas id="sig" class="sig"></canvas><button type="button" class="secondary" onclick="clearSig()">Temizle</button><button class="primary">☁️ Servisi Kaydet</button></form>`);setTimeout(initSig,50)}
...
document.addEventListener("submit",async e=>{if(e.target.id!=="sForm")return;e.preventDefault();const row={user_id:user.id,musteri_id:$("sCustomer").value,servis_tarihi:$("sDate").value,ariza:$("sComplaint").value.trim(),yapilan_islem:$("sDiagnosis").value.trim()+"\n"+$("sWork").value.trim(),degisen_parca:$("sPart").value.trim(),parca_ucreti:Number($("sParts").value||0),iscilik_ucreti:Number($("sLabor").value||0),odeme_durumu:$("sPay").value,servis_durumu:$("sStatus").value,notlar:""};const {data,error}=await db.from("servis_kayitlari").insert(row).select().single();if(error){alert(error.message);return}const files=[...$("sPhoto").files];for(const f of files){const path=`${user.id}/${data.id}/${crypto.randomUUID()}-${f.name}`;const up=await db.storage.from("servis-fotolar").upload(path,f,{upsert:false});if(!up.error){const url=db.storage.from("servis-fotolar").getPublicUrl(path).data.publicUrl;await db.from("servis_fotograflari").insert({user_id:user.id,servis_id:data.id,foto_url:url,aciklama:"Servis fotoğrafı"})}}closeModal();toast("Servis kaydedildi");await loadAll();makePDF(data.id)});
...
const pendingStatuses=["Bekliyor","İşlemde","Parça Bekliyor","Müşteri Onayı Bekliyor"];
window.renderPendingServices=()=>{
 const filter=$("pendingFilter")?.value||"all";
 const arr=services.filter(s=>pendingStatuses.includes(s.servis_durumu||"Bekliyor")).filter(s=>filter==="all"||s.servis_durumu===filter);
 $("pendingCount").textContent=arr.length;
 $("pendingList").innerHTML=arr.map(s=>{const c=customers.find(x=>x.id===s.musteri_id)||{};return `<div class="pending-card"><div class="pending-top"><strong>${esc(c.ad_soyad||"Müşteri")}</strong><span class="badge">${esc(s.servis_durumu||"Bekliyor")}</span></div><div>${esc(c.cihaz_turu||"Cihaz")} · ${esc(c.marka||"")} ${esc(c.model||"")}</div><div class="muted">📅 ${esc(s.servis_tarihi||"-")} · 🔴 ${esc(s.ariza||"Arıza belirtilmedi")}</div><div class="pending-actions"><button onclick="call('${encodeURIComponent(c.telefon||"")}')">📞 Ara</button><button onclick="openPendingDetail('${s.id}')">✏️ Durum</button></div></div>`}).join("")||'<div class="empty">Bekleyen servis yok. 🎉</div>';
};
window.openPendingDetail=id=>{const s=services.find(x=>x.id===id);if(!s)return;const c=customers.find(x=>x.id===s.musteri_id)||{};openModal(`<h2>🔧 Servis İşlemi</h2><div class="card"><b>${esc(c.ad_soyad||"")}</b><div>${esc(c.cihaz_turu||"")} · ${esc(c.marka||"")} ${esc(c.model||"")}</div><div class="muted">${esc(s.ariza||"")}</div></div><label>Servis durumu<select id="editServiceStatus"><option ${s.servis_durumu==="Bekliyor"?"selected":""}>Bekliyor</option><option ${s.servis_durumu==="İşlemde"?"selected":""}>İşlemde</option><option ${s.servis_durumu==="Parça Bekliyor"?"selected":""}>Parça Bekliyor</option><option ${s.servis_durumu==="Müşteri Onayı Bekliyor"?"selected":""}>Müşteri Onayı Bekliyor</option><option ${s.servis_durumu==="Tamamlandı"?"selected":""}>Tamamlandı</option><option ${s.servis_durumu==="İptal"?"selected":""}>İptal</option></select></label><textarea id="editWork" placeholder="Yapılan işlem / servis notu">${esc(s.yapilan_islem||"")}</textarea><button class="primary" onclick="savePendingDetail('${s.id}')">💾 Kaydet</button>`);};
window.savePendingDetail=async id=>{const s=services.find(x=>x.id===id);if(!s)return;const {error}=await db.from("servisler").update({servis_durumu:$("editServiceStatus").value,yapilan_islem:$("editWork").value.trim()}).eq("id",id);if(error){toast("Servis güncellenemedi: "+error.message,true);return;}closeModal();toast("Servis durumu güncellendi");await loadAll();renderPendingServices();};
